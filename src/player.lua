...
-- power bar with fruit-combo mechanic, and per-character special powers.
...
-- Fallback profile for unknown paths
local DEFAULT_PROFILE = CharProfiles["assets/Main Characters/Mask Dude/"]
...
function Player.new(x, y, charPath)
    ...
    local profile = CharProfiles[base] or DEFAULT_PROFILE
    ...
    -- Double-jump (reset to 2 on landing; 3 during Triple Jump special)
    self.jumpsLeft = 2
    ...
    -- ── Character profile ──────────────────────────────────────────────────
    self.moveSpeed    = profile.speed
    self.jumpVel      = profile.jumpVel
    self.favoriteFruit = profile.favoriteFruit
    self.specialFn    = profile.specialFn
    self.specialName  = profile.specialName
    ...
    self.specialTimer    = 0
    self.specialType     = nil   -- "speed" | "triplejump" | "shield" | "scorerush"
...
-- addScore: always applies current score multiplier.
function Player:addScore(amount)
    self.score = self.score + amount * self.scoreMultiplier
end

-- collectFruit: call when player picks up a fruit.
--   fruitType  = filename string ("Apple.png", etc.)
--   baseValue  = base point value of the fruit
-- Tracks combo, fills power bar, and adds score.
function Player:collectFruit(fruitType, baseValue)
    -- Combo tracking: consecutive same-type fruits
    if fruitType == self.lastFruitType then
    ...
    self:addScore(baseValue)
end

-- ─── Special power ────────────────────────────────────────────────────────────

-- activateSpecial: triggers special if bar is full.
-- Returns true if activated.
function Player:activateSpecial()
    if self.powerBar >= 100 and not self.specialActive then
        self.powerBar      = 0
        self.lastFruitType = nil
        self.comboCount    = 0
        self.specialFn(self)
...
function Player:takeDamage()
    ...
    if self.specialType == "shield" then return end   -- shield blocks all hits
...
function Player:update(dt, level)
    ...
    -- Effective movement speed (boosted during Speed Surge)
    local effectiveSpeed = self.moveSpeed
    if self.specialType == "speed" then
        effectiveSpeed = self.moveSpeed * 1.6
    end
    ...
    if collY ~= 0 then
        ...
        if self.vy > 0 then
            self.onGround  = true
            -- Triple Jump special grants maxJumps=3 while active
            self.jumpsLeft = (self.specialType == "triplejump") and 3 or 2
        end
        self.vy = 0
    else
        self.onGround = false
...
function Player:keypressed(key)
    if key == "up" or key == "w" or key == "space" then
        if self.state ~= "hit" and self.jumpsLeft > 0 then
            local maxJumps     = (self.specialType == "triplejump") and 3 or 2
            local isDoubleJump = (not self.onGround and self.jumpsLeft < maxJumps)
...
function Player:draw()
    -- Shield aura: golden circle drawn behind the sprite
    if self.specialType == "shield" then
        love.graphics.setColor(1, 0.85, 0.1, 0.35)
        love.graphics.circle("fill", self.x + 16, self.y + 16, 22)
        love.graphics.setColor(1, 0.85, 0.1, 0.8)
        love.graphics.setLineWidth(2)
        love.graphics.circle("line", self.x + 16, self.y + 16, 22)
